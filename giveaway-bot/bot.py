...
async def create_giveaway_message(
    ...
    embed = giveaway_embed(
        ...
        host=host, required_role_id=role_id,
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?)
        """,
        (message.id, guild_id, channel_id, host.id, prize, min_invites, winners,
         unix, role_id, bonus_every_invites, bonus_entries),
    )
    conn.commit()
    return message
...
async def giveaway_create(
    ...
    message = await create_giveaway_message(
        interaction.channel, interaction.guild_id, interaction.channel_id, interaction.user,
        prize, minutes, min_invites, winners, required_role, bonus_every_invites, bonus_entries,
    )
...
@app_commands.checks.has_permissions(manage_guild=True)
async def giveaway_reroll(interaction: discord.Interaction, message_id: str, winners: app_commands.Range[int, 1, 20] = 1):
    if not message_id.isdigit():
        return await interaction.response.send_message("Invalid message ID.", ephemeral=True)
...
async def giveaway_blacklist(interaction: discord.Interaction, member: discord.Member, reason: str = "No reason provided"):
    conn.execute("INSERT OR REPLACE INTO blacklist (guild_id, user_id, reason) VALUES (?, ?, ?)", (interaction.guild_id, member.id, reason[:300]))
    conn.commit()
...
async def dot_help(ctx: commands.Context):
    ...
    embed.description = (
        ...
        "Advanced role + bonus-entry settings are also available with `/giveaway_create`."
    )
...
async def dot_invites(ctx: commands.Context, member: discord.Member | None = None):
    ...
    count = get_invite_count(ctx.guild.id, target.id)
    await ctx.send(f"📨 {target.mention} has **{count} valid invite{'s' if count != 1 else ''}**.")


@bot.command(name="giveaway", aliases=["gcreate"])
@commands.has_guild_permissions(manage_guild=True)
async def dot_giveaway(ctx: commands.Context, minutes: int, min_invites: int, winners: int, *, prize: str):
    if not ctx.guild:
        return
    ...
    message = await create_giveaway_message(
        ...
        prize, minutes, min_invites, winners,
    )
...
@commands.has_guild_permissions(manage_guild=True)
async def dot_blacklist(ctx: commands.Context, member: discord.Member, *, reason: str = "No reason provided"):
    conn.execute("INSERT OR REPLACE INTO blacklist (guild_id, user_id, reason) VALUES (?, ?, ?)", (ctx.guild.id, member.id, reason[:300]))
    conn.commit()
...
async def dot_blacklist_check(ctx: commands.Context, member: discord.Member):
    row = conn.execute("SELECT reason FROM blacklist WHERE guild_id = ? AND user_id = ?", (ctx.guild.id, member.id)).fetchone()
    if row:
